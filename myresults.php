<?php
 try {
     ...
     // // Get vote counts for the specific token
     // $yes_votes = $db->querySingle("SELECT COUNT(*) FROM polls WHERE vote = 'yes' AND token = '$token'");
     // $no_votes = $db->querySingle("SELECT COUNT(*) FROM polls WHERE vote = 'no' AND token = '$token'");
     // $total_votes = $yes_votes + $no_votes;
     // $pending_votes = $db->querySingle("SELECT COUNT(*) FROM polls WHERE vote IS NULL AND token = '$token'");
    ...
    $no_percentage = $total_votes > 0 ? round(($no_votes / $total_votes) * 100, 1) : 0;
     ...
     $results = $db->query("SELECT token, vote, voted_at FROM polls WHERE token = '$token' ORDER BY voted_at DESC");
     if (!$results) {
